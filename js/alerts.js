...
// Server filters toggle
/* ============================================
   FILTERS - Server Filters Management (see js/filters-card.js)
   ============================================ */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const savedFiltersValue = document.getElementById('saved-filters-value');
    if (savedFiltersValue) {
        updatePageTitle(savedFiltersValue.textContent);
    }
...
// ── Notifications panel ──
const H = 3600000, D = 86400000, N = Date.now();
let notifList = [
    { id: 1, cat: 'trade', time: N - 2 * 60 * 1000, read: false, icon: 'fas fa-arrows-rotate', iconCls: 'trade', title: 'Trade Workflow Updated', msg: 'TRD-4821 moved to Confirmed status', link: 'View Trade →' },
    { id: 2, cat: 'trade', time: N - 15 * 60 * 1000, read: false, icon: 'fas fa-check', iconCls: 'trade', title: 'Trade Confirmed', msg: 'TRD-4819 confirmed by operations', link: 'View Trade →' },
    { id: 3, cat: 'trade', time: N - H, read: false, icon: 'fas fa-lock', iconCls: 'trade', title: 'Bulk Lock Updated', msg: '42 trades locked until 1 Mar 2026', link: '' },
    { id: 4, cat: 'trade', time: N - 3 * H, read: true, icon: 'fas fa-right-left', iconCls: 'trade', title: 'Interbook Trade Transfer', msg: 'TRD-4801 moved from Alpha to Beta book', link: 'View Trade →' },
    { id: 5, cat: 'upload', time: N - 3.5 * H, read: false, icon: 'fas fa-file', iconCls: 'upload-err', title: 'Trade Upload Failed', msg: 'curves_nymex.xlsx — 3 rows could not be processed', link: 'View errors →', err: true },
    { id: 6, cat: 'upload', time: N - 4 * H, read: true, icon: 'fas fa-file', iconCls: 'upload', title: 'Trade Upload Complete', msg: 'trades_feb24.csv — 120 rows processed', link: '' },
    { id: 7, cat: 'valuation', time: N - 5 * H, read: false, icon: 'fas fa-triangle-exclamation', iconCls: 'valuation', title: 'Valuation Error', msg: 'TRD-4802 could not be valued — missing curve data', link: 'View Trade →', err: true },
    { id: 8, cat: 'market', time: N - D - 2 * H, read: false, icon: 'fas fa-chart-line', iconCls: 'market', title: 'CME Busted Trade', msg: 'TRD-4790 was busted by the exchange', link: 'View Trade →' },
    { id: 9, cat: 'system', time: N - 3 * D, read: true, icon: 'fas fa-sparkles', iconCls: 'system', title: "What's new — Market Data v3", msg: 'Notification inbox, curves and volatilities improvements', link: 'Read →' }
];
let notifNextId = 10, notifPanelOpen = false, notifActiveTab = 'all', notifMenuOpen = false, notifFilterOpen = false;
const NOTIF_TAB_CAT = { trade: 'trade', market: 'market', valuation: 'valuation', others: 'upload' };

function notifFmtTime(ts) {
    var diff = Date.now() - ts, m = Math.floor(diff / 60000), h = Math.floor(diff / H);
    if (m < 1) return 'Just now';
    if (m < 60) return m + 'm ago';
    if (h < 24) return h + 'h ago';
    var d = new Date(ts), t = new Date(), y = new Date(Date.now() - D);
    if (d.toDateString() === t.toDateString()) return 'Today ' + d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
    if (d.toDateString() === y.toDateString()) return 'Yesterday ' + d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
    return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
}

function notifSortFn(s) {
    return function (a, b) {
        if (s === 'newest') return b.time - a.time;
        if (s === 'oldest') return a.time - b.time;
        if (s === 'unread-first') return a.read !== b.read ? (a.read ? 1 : -1) : b.time - a.time;
        return b.time - a.time;
    };
}

function notifItemHTML(n) {
    var uc = n.read ? 'off' : '', rc = n.read ? '' : 'unread', mc = n.err ? 'err' : '';
    var iconErrCls = n.err ? ' err' : '';
    var lk = n.link ? '<span class="notif-item-link">' + n.link + '</span>' : '<span></span>';
    var iconHtml = n.icon.indexOf('fa-') !== -1 ? '<i class="' + n.icon + '"></i>' : '<span>' + n.icon + '</span>';
    return '<div class="notif-item ' + rc + '" data-id="' + n.id + '" onclick="notifMarkRead(' + n.id + ', this)">' +
        '<div class="notif-unread-dot ' + uc + '"></div>' +
        '<div class="notif-item-icon ' + n.iconCls + iconErrCls + '">' + iconHtml + '</div>' +
        '<div class="notif-item-content">' +
        '<div class="notif-item-title">' + n.title + '</div>' +
        '<div class="notif-item-msg ' + mc + '">' + n.msg + '</div>' +
        '<div class="notif-item-footer">' +
        '<span class="notif-item-time">' + notifFmtTime(n.time) + '</span>' + lk +
        '</div></div>' +
        '<button type="button" class="notif-del-btn" onclick="event.stopPropagation(); notifDeleteOne(' + n.id + ')" title="Delete">×</button></div>';
}

function renderNotifList() {
    var show = document.getElementById('notif-show-filter').value;
    var sort = document.getElementById('notif-sort-by').value;
    var items;
    if (notifActiveTab === 'all') {
        items = show === 'unread' ? notifList.filter(function (n) { return !n.read; }) : notifList.slice();
    } else {
        var cat = NOTIF_TAB_CAT[notifActiveTab] || notifActiveTab;
        items = notifList.filter(function (n) { return n.cat === cat; });
        if (show === 'unread') items = items.filter(function (n) { return !n.read; });
    }
    items.sort(notifSortFn(sort));
    var html = items.map(notifItemHTML).join('');
    if (!html) {
        html = '<div class="notif-empty-state">' +
            ...
            '<div class="notif-empty-sub">' + (show === 'unread' ? "You're all caught up!" : 'Nothing here yet.') + '</div></div>';
    } else {
        html += '<div class="notif-load-more" onclick="alert(\'Load older notifications\')">Load older notifications</div>';
    }
    document.getElementById('notif-panel-body').innerHTML = html;
    notifUpdateBadge();
    notifUpdateTabCounts();
}

function notifUpdateBadge() {
    var c = notifList.filter(function (n) { return !n.read; }).length;
    ...
    if (pill) { pill.textContent = c; pill.style.display = c ? 'flex' : 'none'; }
}

function notifUpdateTabCounts() {
    function setTC(id, count) {
        var el = document.getElementById(id);
        if (!el) return;
        el.textContent = count > 9 ? '9+' : count;
        el.classList.toggle('show', count > 0);
    }
    setTC('ntc-all', notifList.filter(function (n) { return !n.read; }).length);
    setTC('ntc-trade', notifList.filter(function (n) { return n.cat === 'trade' && !n.read; }).length);
    setTC('ntc-market', notifList.filter(function (n) { return n.cat === 'market' && !n.read; }).length);
    setTC('ntc-valuation', notifList.filter(function (n) { return n.cat === 'valuation' && !n.read; }).length);
    setTC('ntc-others', notifList.filter(function (n) { return n.cat === 'upload' && !n.read; }).length);
}
...
document.addEventListener('click', function (e) {
    ...
    var bell = document.querySelector('.bell-wrapper');
    if (notifPanelOpen && panel && !panel.contains(e.target) && (!bell || !bell.contains(e.target))) {
        notifPanelOpen = false;
        panel.classList.remove('open');
    }
    if (notifFilterOpen && !document.getElementById('notif-filter-wrap').contains(e.target)) { notifFilterOpen = false; document.getElementById('notif-filter-dropdown').classList.remove('open'); }
    if (notifMenuOpen && !document.getElementById('notif-menu-wrap').contains(e.target)) { notifMenuOpen = false; document.getElementById('notif-menu-dropdown').classList.remove('open'); }
});

function toggleNotifFilter() {
    notifFilterOpen = !notifFilterOpen;
    document.getElementById('notif-filter-dropdown').classList.toggle('open', notifFilterOpen);
    if (notifFilterOpen) closeNotifMenu();
}
function toggleNotifMenu() {
    notifMenuOpen = !notifMenuOpen;
    document.getElementById('notif-menu-dropdown').classList.toggle('open', notifMenuOpen);
    if (notifMenuOpen) {
        notifFilterOpen = false;
        document.getElementById('notif-filter-dropdown').classList.remove('open');
    }
}
function closeNotifMenu() {
    notifMenuOpen = false;
    var el = document.getElementById('notif-menu-dropdown');
    if (el) el.classList.remove('open');
}

function notifMarkRead(id, el) {
    var n = notifList.find(function (x) { return x.id === id; });
    if (n) n.read = true;
    if (el) { el.classList.remove('unread'); var d = el.querySelector('.notif-unread-dot'); if (d) d.classList.add('off'); }
    notifUpdateBadge();
    notifUpdateTabCounts();
}
...
function notifDeleteOne(id) {
    notifList = notifList.filter(function (n) { return n.id !== id; });
    renderNotifList();
}
function notifDeleteRead() {
    notifList = notifList.filter(function (n) { return !n.read; });
    renderNotifList();
}
function notifDeleteAll() {
